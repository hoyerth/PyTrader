...
## 16.01.02 ⚠️ Nachgelagerte Arbeiten - NICHT AUTOMATISCH AUSFÜHREN!
...
* **Service-Sets & Presets (`indicator_presets`, `service_sets`):**
  ...
  * *Endgültige Bereinigung:* Einmaliges manuelles Öffnen und Re-Speichern der betroffenen Service-Sets im `ServiceWindow` zur Bereinigung der JSON-Payloads in der Datenbank.

### 3. Verifikation & Test-Cleanup
* **Isolierter Test-Check:**
  * Ausführung von `test/check_p16_s1_decoupling.py` zur Validierung, dass `feature_store_payload` frei von Canvas-Objekten bleibt und `FixedGridProximityIndicator` das `chart_render_payload` korrekt baut.
  * Statische Syntaxprüfung via `python -m py_compile`.